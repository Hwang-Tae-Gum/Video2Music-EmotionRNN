...
def main():
    ...
    # VideoRegression은 emotion을 concat 방식으로 처리 (별도 dim)
    # VideoMusicTransformer는 Linear_emo 별도 경로로 처리하므로 total_vf_dim에 불포함
    if args.emo_model.startswith("6c"):
        reg_vf_dim = total_vf_dim + 6
    else:
        reg_vf_dim = total_vf_dim + 5
    ...
    if args.is_video:
        ...
        model.load_state_dict(torch.load(args.model_weights, map_location=get_device(), weights_only=False))
        ...
        modelReg.load_state_dict(torch.load(args.modelReg_weights, map_location=get_device(), weights_only=False))
    else:
    ...
    with torch.set_grad_enabled(False):
        if(args.beam > 0):
            ...
            # Convert midi to audio (e.g., flac) — find fluidsynth/sf2 dynamically
            _sf2_candidates = [
