...
train_full.py
─────────────
Baseline (full): 모든 feature (semantic / scene_offset / motion / emotion) 사용.
train_no_emotion.py 와 동일한 조건(BATCH_SIZE=4, LR schedule)으로 학습해
공정한 ablation 비교를 위한 체크포인트를 생성한다.

출력: ./saved_models/AMT_full/
사용: python train_full.py
"""
...
OUTPUT_DIR  = "./saved_models/AMT_full"
...
def main():
    ...
    total_vf_dim  = sum(vf.shape[1] for vf in train_ds[0]["semanticList"])
